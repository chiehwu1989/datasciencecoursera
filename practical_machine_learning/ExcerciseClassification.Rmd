...
```
None of the categorical variables will be selected for training the model because of the following reasons:
* Besides user_name, most of the categorical variables seems to be continuous variables, but the majority is unknown value such as empty string and "#DIV/0!". 
* Chisq test between user_name and classe gives a very low p-value (3.67e-40) so user_name and classe are independent.
## Model Training
Will train the model using randome forest with 10-fold cross validation.
```{r trainModel}
finalTraining <- trainingNonNA %>%
  select(one_of(lowCorColName))
